...
-- --------------------------------------------------
-- Date Created: 09/06/2012 14:21:22
...
CREATE TABLE [dbo].[ConversationHistories] (
    ...
    [Conversation_ConvId] nvarchar(50)  NOT NULL
);
GO
...
-- Creating foreign key on [Conversation_ConvId] in table 'ConversationHistories'
ALTER TABLE [dbo].[ConversationHistories]
ADD CONSTRAINT [FK_ConversationToConversationHistory]
    FOREIGN KEY ([Conversation_ConvId])
    REFERENCES [dbo].[Conversations]
        ([ConvId])
    ON DELETE NO ACTION ON UPDATE NO ACTION;
