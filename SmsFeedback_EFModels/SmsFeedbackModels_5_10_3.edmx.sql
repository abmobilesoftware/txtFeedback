...
-- --------------------------------------------------
-- Date Created: 05/10/2013 18:12:47
...
CREATE TABLE [dbo].[UserDevice] (
    [Users_UserId] uniqueidentifier  NOT NULL,
    [Devices_Id] nvarchar(max)  NOT NULL
);
GO
...
-- Creating primary key on [Users_UserId], [Devices_Id] in table 'UserDevice'
ALTER TABLE [dbo].[UserDevice]
ADD CONSTRAINT [PK_UserDevice]
    PRIMARY KEY NONCLUSTERED ([Users_UserId], [Devices_Id] ASC);
GO
...
-- Creating non-clustered index for FOREIGN KEY 'FK_UserDevice_Device'
CREATE INDEX [IX_FK_UserDevice_Device]
ON [dbo].[UserDevice]
    ([Devices_Id]);
GO
